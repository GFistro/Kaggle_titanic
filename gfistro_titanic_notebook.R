...
join_table[(is.na(join_table$Age.kaggle) & is.na(join_table$Age.wiki)), ] #In total we have 131 cases, where we dont have the data in the originial dataset, nor was there found a match in the wikipedia one
...
titanic_data_clean[titanic_data_clean$Ticket == 349909, ]
...
table(titanic_data_clean$Group_ID)

titanic_data_clean[titanic_data_clean$Group_ID == 50, ]
...
titanic_data_clean[titanic_data_clean$Ticket == 2627, ]
...
titanic_data_clean <- titanic_data_clean %>%
  mutate(Embarked = as.factor(ifelse(is.na(Embarked), "S", as.character(Embarked))),
         ...
         train = as.factor(train)) #We also fill in the 2 NA's from Embarked with the most common value, and the missing fare_real value


knnpp <- preProcess(as.data.frame(titanic_data_clean[, -1]), method = "knnImpute")
titanic_data_clean <- predict(knnpp, titanic_data_clean)

which(join_table$Age == 15)
join_table[23,]
kid_age <- titanic_data_clean[23,]$Age

titanic_data_clean$child <- as.factor(ifelse(titanic_data_clean$Age <= kid_age, 1, 0)) #We also define a new variable children, which is 1 for every person under 15 years old
...
ggplot(age_survival, aes(x = Age, y = survival)) +
  ...
  geom_vline(xintercept = kid_age) + #Before the cutoff at age 15, we see some higher survival rates 
...
algorithm_list <- c("xgbTree", "glmnet", "ranger", "bayesglm", "avNNet", "glmboost", "ada", "svmRadialSigma", "knn")


#Training on whole training data
final_dat <- mod_train
final_dat <- final_dat %>%
  select(-c(cabin_letter, dup_tic, child))
...
final_algorithm_list <- c("xgbTree", "avNNet", "glmnet", "svmRadialSigma")
...
mean(testpreds2 == as.numeric(final_dat$Survived) - 1) #90.7% Accuracy on the training set
auc(as.numeric(final_dat$Survived) - 1 , testpreds2) #0.896 AUC
...
write.csv(submission_data, file = "gfistro_titanic_notebook_GS3.csv", row.names = F)
